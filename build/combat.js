...
function processCombatPage() {
    ...
    if (combatUIIsPrepared) {
        ...
        if (!skillContainers)
            skillContainers = queryEncryptedAll(document, "div.prt-command-chara:not(.quick-panels)");
...
function processAutoAdvance() {
    ...
    if (suppressAutoSkip) {
        if (!loggedSuppressedSkipMessage) {
            logError("Another window is already navigating to the results screen.");
...
function prepareCombatUI() {
    ...
    waitForElementToExist(document, "div.contents", function (contents) {
        // We need to create this element early for the tnum handler to get attached
        ensureQuickPanelsContainerExists();
        ensureQuickSummonPanelExists();
    }, true);
    waitForElementToExist(document, "div.prt-command", function (contents) {
        // The container may have been created before prt-command appeared so we move it
        ensureQuickPanelsContainerExists();
        ensureQuickSummonPanelExists();
    }, true);
    waitForElementToExist(document, "div.cnt-raid-stage", function (stageWrapper) {
        ...
        if (raidContainer && currentSettings.largeQuickPanels)
            addEncryptedClass(raidContainer, "large-quick-panels");
...
function setupStickerUI() {
    ...
    waitForElementToExist(document, "div.pop-usual.pop-ready-stamp", function (popup) {
        ...
        searchField = queryEncrypted(popup, "input.stamp-filter");
        if (searchField)
            return;
        body = popup.querySelector("div.txt-popup-body");
        searchField = document.createElement("input");
        searchField.className = encryptClassName("stamp-filter");
...
function updateQuickPanelSize() {
    if (!raidContainer)
        return;
    var cn = encryptClassName("large-quick-panels");
