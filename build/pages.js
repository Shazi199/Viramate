...
var currentPageIsCoop = false, lastPageWasCoop = false;
function handleNewPage() {
    ...
    lastPageWasCoop = currentPageIsCoop;
    currentPageIsCoop = location.hash.indexOf("coop") >= 0;
...
;
var activeHorizontalWatches = new Set();
var questLogStyleElement = null;
var questLogStyleText = null;
function createQuestLogStyleElement() {
    if (questLogStyleElement)
        return;
    if (!questLogStyleText)
        questLogStyleText = "";
    var sts = currentSettings.storyTextScale;
    if ((typeof (sts) === "number") &&
        (sts <= 0.99) || (sts >= 1.01))
        questLogStyleText += "\n.prt-scene-comment .prt-message-area .txt-message { " +
            "font-size: " + (sts * 14).toFixed(1) +
            "px !important; }\n";
    questLogStyleElement = document.createElement("style");
    questLogStyleElement.textContent = questLogStyleText;
    if (maskElements)
        questLogStyleElement.setAttribute("_vm", "qls");
    document.body.appendChild(questLogStyleElement);
}
;
function handleNewPageInner() {
    ...
    try {
        ...
        var gc = getGameContainer();
        if (!isCombatPage(hash)) {
            var dt = uic.querySelector("div.debuff-timers");
            if (dt)
                uic.removeChild(dt);
        }
        if (isHorizontalLayout()) {
            var ids = ["div#submenu", "div#chat-body"];
            ids.forEach(function (id) {
                if (activeHorizontalWatches.has(id))
                    return;
                activeHorizontalWatches.add(id);
                waitForElementToExist(gc, id, function (elt) {
                    activeHorizontalWatches.delete(id);
                    var effectiveZoom = getEffectiveZoom(gc);
                    elt.style.paddingTop = (48 / effectiveZoom).toFixed(0) + "px";
                }, false, false);
            });
            if (!questLogStyleElement) {
                var effectiveZoom = getEffectiveZoom(gc);
                questLogStyleText = "div.cnt-quest-scene.log div.prt-quest-header.scene {\n" +
                    "  margin-top: " + (48 / effectiveZoom).toFixed(0) + "px;\n" +
                    "}";
            }
        }
    }
    catch (exc) {
        logError("Error in handleNewPageInner", exc);
    }
    window.setTimeout(function () {
    ...
    if (currentSettings.autofocusDropdowns &&
        (hash.indexOf("shop/") >= 0) ||
        (hash.indexOf("treasure") >= 0)) {
        waitForElementToExist(document, "div.pop-usual select.num-set", function (dropdown) {
            dropdown.focus();
        }, true);
    }
    ...
    else if (window.location.hash.search(/event\/teamraid[\d]+(\/top)?$/) >= 0) {
        processGuildWarPage();
    }
    else {
...
function _showWatchButton(element, itemId, isActive) {
    ...
    watchButton.className = encryptClassName("watch-button") +
        (isActive ? " active" : "");
...
function showWatchButtons(containerSelector, itemSelector, getItemId) {
    ...
    if (!mostRecentItems) {
        getUserIdAndTabIdAsync(function (uid, tabId) {
            getRecentItems(null, uid, doInit, false);
        });
    }
    else
        doInit();
...
var itemUrlParseRe = /item\/article\/[sml]\/([0-9]+)\..+/;
function getItemIdFromImageUrl(treasure) {
    var itemImage = treasure.querySelector("img").src;
    var m = itemUrlParseRe.exec(itemImage);
    if (!m)
        return;
...
function processItemPage() {
    showItemWatchButtons = function () {
        showWatchButtons("div.prt-item-list div.prt-item-filter", "div.lis-item", getItemIdFromImageUrl);
    };
}
;
...
function processExchangeShopPage() {
    var getItemId = function (itemIcon) {
        if (!itemIcon.hasAttribute("data-item-id"))
            return null;
        var ppp = (itemIcon.parentNode || {});
        ppp = ppp.parentNode || {};
        ppp = ppp.parentNode || {};
        ppp = ppp.parentNode || {};
        ppp = ppp || "";
        // HACK
        if (ppp.className.indexOf("twobytwo") < 0)
...
function processCoOpPage() {
    waitForElementToExist(document, "div.prt-wanted-room.btn-wanted-room", function (roomButton) {
        var questId = roomButton.getAttribute("data-quest-id");
        if (!questId || Number.isNaN(parseInt(questId)) || questId === "0")
            return;
        ...
        if (i18n.get("lang") === "en")
            ...
            imageUrl += "assets";
        imageUrl += "/img/sp/quest/assets/lobby/" + questId + ".png";
        portraitImg.src = imageUrl;
        portraitImg.style.float="right";
        portraitImg.style.width="auto";
        portraitImg.style.height="40px";
        portraitImg.style.marginTop="3px";
        portraitImg.style.marginRight="21px";
        roomButton.prepend(portraitImg);
        var comment = roomButton.querySelector("div.txt-room-comment");
        if (comment)
            comment.style.maxWidth = "240px";
    }, true);
    waitForElementToExist(document, "div.pop-usual.pop-room-detail", function (joinPopup) {
        var raidDetail = joinPopup.querySelector("div.prt-base-data-lobby.high") ||
            joinPopup.querySelector("div.prt-base-data-lobby.normal");
        if (raidDetail) {
            if (!joinPopup.querySelector("div.ico-continue-quest"))
                return;
        }
...
function processGuildWarPage() {
    var shortHref = window.location.hash.substr(1).replace("/top", "");
    sendExtensionMessage({ type: "setCurrentGuildWar", href: shortHref });
    var parser = new DOMParser();
    var allRaidInfo = Object.create(null);
    var processTemplate = function (isHell, templateElement) {
        var templateFragment = parser.parseFromString(templateElement.textContent, "text/html");
        var buttons = Array.from(templateFragment.querySelectorAll(".btn-multi-battle") || []);
        buttons = buttons.concat(Array.from(templateFragment.querySelectorAll(".btn-offer") || []));
        if (buttons.length === 0)
            return;
        for (var j = 0; j < buttons.length; j++) {
            ...
            var raidInfo = {
                ...
                id: chapterId,
                // gross
                isHell: isHell || (button.innerHTML.indexOf("_hell") >= 0)
            };
    ...
    waitForElementToExist(document, "script[type='text/template']", processTemplate.bind(null, false), true);
    waitForElementToExist(document, "script#tpl-button-battle-hell", processTemplate.bind(null, true), true);
}
;
...
var preferredAttribute = null;
function parseSupporterListData(dataString) {
    var regex = /class%3D%22icon-supporter-type-([0-9]+)%20btn-type%20selected%22%20data-type%3D%22([0-9]+)%22/;
    var attrs = dataString.match(regex);
    if (!attrs || attrs.length < 3) {
        log("Parsing supporter list data failed");
        preferredAttribute = null;
        return;
    }
    preferredAttribute = attrs[2];
    log("Parsed preferred attribute from network", preferredAttribute);
    refreshPreferredAttribute();
}
;
function refreshPreferredAttribute() {
    if (!currentSettings.highlightSummons)
        return;
    if (preferredAttribute === null)
        return;
    var container = document.querySelector("div.prt-supporter-list.prt-module");
    addEncryptedClass(container, "highlight-attribute-" + preferredAttribute);
}
;
function processSupporterPage() {
    storedSupportsPage = null;
    preferredAttribute = null;
    var clsFaved = encryptClassName("faved");
    ...
    waitForElementToExist(document, "div.prt-supporter-attribute", function (supporterList) {
        if (supporterList.className.indexOf(clsFaved) >= 0)
            return;
        processSupporterList(currentSettings, supporterList);
    }, true);
    waitForElementToExist(document, "div.btn-type div.prt-type-text", function (tabButton) {
        var tabAttribute = tabButton.parentNode.getAttribute("data-type");
        if (tabAttribute) {
            var isSelected = (tabButton.className.indexOf(" selected") >= 0);
            if ((preferredAttribute === null) && isSelected)
                preferredAttribute = tabAttribute;
            if (tabAttribute == preferredAttribute)
...
function fillQuestInfoBalloon(balloon) {
    ...
    var isEvent = false, isCoop = false;
    if (!currentSettings.showQuestInfo)
        return;
    if (pageUrl.indexOf("arcarum2/") >= 0) {
        log("Not loading quest info for arcarum");
        return;
    }
    // HACK
    if (parts[0] === "#event") {
    ...
    isCoop = (parts[1] === "supporter_lobby") || lastPageWasCoop;
    if ((parts[1] !== "supporter") && !isCoop) {
    ...
    if (parts.length > 4 && Math.floor(questId / 100000) === 3) {
        ...
        var treasureCallback = function (questInfo, error) {
            if (!questInfo) {
                log("Failed to get quest info", error);
                sendExtensionMessage({ type: "setCachedQuestInfo", key: url, value: null });
                return;
            }
            else if (!questInfo.__cached__ &&
                !questInfo.consume &&
                !questInfo.treasure_id) {
                sendExtensionMessage({ type: "setCachedQuestInfo", key: url, value: questInfo });
            }
        ...
        url = "/quest/treasure_raid/" + chapterId + "/" + questId;
        sendExtensionMessage({ type: "getCachedQuestInfo", key: url }, function (result) {
            if (!result) {
                log("Loading quest info from network");
                doClientAjax(url, treasureCallback);
            }
            else {
                log("Loaded quest info from cache");
                result.__cached__ = true;
                treasureCallback(result, null);
            }
        });
    }
    else {
        log((isEvent ? "Event" : "ordinary") + " raid start page", questId);
        var raidCallback = function (questInfo, error) {
            if (!questInfo) {
                log("Failed to get quest info", error);
                sendExtensionMessage({ type: "setCachedQuestInfo", key: url, value: null });
                return;
        ...
        url = "/quest/quest_data/" + questId + "/" + questType;
        sendExtensionMessage({ type: "getCachedQuestInfo", key: url }, function (result) {
            if (!result) {
                log("Loading quest info from network");
                doClientAjax(url, raidCallback);
            }
            else {
                log("Loaded quest info from cache");
                result.__cached__ = true;
                raidCallback(result, null);
            }
        });
    }
}
...
function processSmartSupports() {
    ...
    var isCoop = (window.location.href.indexOf("supporter_lobby") > 1) || lastPageWasCoop;
    var riskSelectors = [
        ".prt-supporter-confirm",
        ".prt-check-auth",
        ".prt-disabled-auth",
        ".txt-confirm-comment",
        ".prt-supporter-battle-announce",
        ".btn-supporter-battle-announce"
    ];
    var safetyCheck = function () {
        for (var sel of riskSelectors) {
            if (document.querySelector(sel)) {
                logError("Verify prompt might be up so no faved tab");
                return true;
            }
        }
        var unknownElts = queryEncryptedAll(document, "div.prt-supporter-list div.prt-supporter-attribute:not(.faved) > :not(.lis-supporter)");
        if (unknownElts.length) {
            if ((unknownElts.length === 1) &&
                (unknownElts[0].nodeName.toUpperCase() === "DIV") &&
                (unknownElts[0].className === encryptClassName("no-faves-message")))
                return false;
            logError("Unknown elements in supporter list: ", unknownElts);
            return true;
        }
        var failedEltCheck = false;
        var questionableElts = document.querySelectorAll("div.prt-supporter-list > *");
        for (var elt of questionableElts) {
            if (elt.nodeName.toUpperCase() !== "DIV") {
                logError("Unknown element:", elt);
                failedEltCheck = true;
                break;
            }
            if ((elt.id === "prt-type") && (elt.className === "")) {
                var child = elt.querySelector(":not(.btn-type)");
                if (!child)
                    continue;
                if ((child.className === "prt-type-text") && (child.children.length === 0))
                    continue;
                logError("Unknown child of prt-type:", child);
                failedEltCheck = true;
                break;
            }
            if ((elt.className === "prt-supporter-title") && (elt.children.length === 0))
                continue;
            if ((elt.className === "onm-frame-line") && (elt.innerHTML === ""))
                continue;
            if (elt.classList.contains("prt-supporter-list") &&
                elt.classList.contains("prt-module"))
                continue;
            // We already scan these lists for weird child elements above
            if (elt.classList.contains("prt-supporter-attribute"))
                continue;
            logError("Unknown element:", elt);
        }
        if (failedEltCheck)
    ...
    var onFavedClick = function (e) {
        ...
        favedView.className = "prt-supporter-attribute " + encryptClassName("faved");
    };
    var maybeChangeTab = function () {
        ...
        if (favedView.firstElementChild.className === encryptClassName("no-faves-message")) {
            selectPreferredSummonTab();
            return;
        }
        if (preferredAttribute === null)
    ...
    var onFavButtonClick = function (e) {
        ...
        orderTable[summonId] = Object.keys(orderTable).length;
    ...
    var onOrderButtonClick = function (e) {
        ...
        var orderTable = getSummonOrderTable();
        var orderedSummons = Object.keys(orderTable);
        orderedSummons.sort(function (lhs, rhs) {
            var lhsOrder = orderTable[lhs];
            if (!lhsOrder)
                lhsOrder = 0;
            var rhsOrder = orderTable[rhs];
            if (!rhsOrder)
                rhsOrder = 0;
        ...
        var fromIndex = orderedSummons.indexOf(summonId);
        var newIndex = fromIndex + delta;
        if (newIndex < 0)
            newIndex = 0;
        else if (newIndex > orderedSummons.length)
            newIndex = orderedSummons.length;
        orderedSummons.splice(fromIndex, 1);
        orderedSummons.splice(newIndex, 0, summonId);
        var newOrderTable = {};
        for (var i = 0; i < orderedSummons.length; i++)
            newOrderTable[orderedSummons[i]] = i;
        currentSettings.summonOrder = newOrderTable;
        sendExtensionMessage({ type: "setSummonOrder", data: JSON.stringify(newOrderTable) }, function (newFaves) {
            refreshFavedView();
        });
    };
    var refreshFavedStates = function () {
        var buttons = queryEncryptedAll(document, "div.fav-summon-button");
        for (var i = 0; i < buttons.length; i++) {
            var btn = buttons[i];
            var id = btn.getAttribute("summon-id");
            btn.className = encryptClassName("fav-summon-button") +
    ...
    var refreshFavedView = function () {
        ...
        if ((items.length < (7 * 6)) && !isCoop) {
            logError("Suspiciously small number of support summons available...", items.length);
            return;
        }
        var parsedItems = [];
        for (var i = 0; i < items.length; i++) {
            ...
            if (parsed === null) {
                logError("Failed to parse supporter", item);
                return;
            }
        ...
        parsedItems.sort(function (lhs, rhs) {
            ...
            // HACK: Fire summons from the misc tab won't get the fire highlight,
            //  so we prefer ones from the fire tab
            if (result === 0)
                result = parseInt(lhs.tabId) - parseInt(rhs.tabId);
        ...
        for (var i = 0; i < parsedItems.length; i++) {
            ...
            addEncryptedClass(copy, "faved");
            var favButton = queryEncrypted(copy, "div.fav-summon-button");
            if (favButton)
                favButton.parentNode.removeChild(favButton);
            var reorderWidget = document.createElement("div");
            reorderWidget.className = encryptClassName("reorder-widget");
            var currentOrder = orderTable[item.id] || 0;
            var orderUp = document.createElement("a");
            orderUp.textContent = "⇧";
            orderUp.title = "Move up";
        ...
        if (parsedItems.length === 0) {
            ...
            message.className = encryptClassName("no-faves-message");
    ...
    sendExtensionMessage({ type: "getFavedSummons" }, function (_favedSummons) {
        ...
        waitForElementToExist(document, "div#prt-type", function (typeStrip) {
            if (safetyCheck())
                return;
            if (queryEncrypted(document, "div.icon-supporter-type-f"))
                return;
            var btn = favedButton = document.createElement("div");
            btn.className = encryptClassName("icon-supporter-type-f") + " btn-type unselected";
            var text = document.createElement("div");
            text.className = "prt-type-text";
            text.textContent = "Faves";
            btn.appendChild(text);
            injectElement(typeStrip, btn);
        ...
        waitForElementToExist(document, [
            "div.prt-supporter-list.prt-module",
            "div#cnt-sequenceraid-supporter > div.prt-supporter-list"
        ], function (supporterList) {
            if (safetyCheck())
                return;
            var view = favedView = document.createElement("div");
            view.className = "prt-supporter-attribute disableView";
            injectElement(supporterList, view);
            refreshFavedView();
            b = true;
            if (a)
                setTimeout(maybeChangeTab, 10);
        ...
        waitForElementToExist(document, "div.btn-supporter.lis-supporter", function (item) {
            var btn = queryEncrypted(item, "div.fav-summon-button");
            if (btn)
                return;
            ...
            btn.addEventListener("mousedown", suppress, true);
            btn.addEventListener("mouseup", suppress, true);
            btn.addEventListener("touchstart", onFavButtonClick.bind(btn), true);
...
function parseSupporter(elt) {
    ...
    result.tabId = elt.getAttribute("data-attribute");
    ...
    if ((computedStyle.display === "none") ||
        ...
        logError("Suspiciously invisible support summon element", elt);
        return null;
    }
    return result;
}
...
function processSupporterList(settings, list) {
    ...
    if (supporters.length === 0)
        return;
    if (supporters[0].className.indexOf(encryptClassName("faved")) >= 0)
        return;
    ...
    for (var i = 0, l = parsedSupporters.length; i < l; i++) {
        ...
        if (currentSettings.hyperlinkUserNames && !ps.element.querySelector("a")) {
...
function showDeckNames(decks) {
    ...
    // FIXME: Encrypt this attribute name
...
function processEnhancementPage() {
    showWatchButtons("div.prt-item-list", "div.prt-item > div.prt-item-img", getItemIdFromImageUrl);
...
;
var currentJoinWaitInterval = null;
var currentJoinTimeout = null;
var currentCoOpJoinWait = null;
var currentRaidJoinWait = null;
var currentNavigationProbe = null;
function getNavigationProbe() {
    if (!currentNavigationProbe)
        return null;
    if (!document.body.contains(currentNavigationProbe))
        return null;
    return currentNavigationProbe;
}
;
function createNavigationProbe() {
    var probe = getNavigationProbe();
    if (!probe) {
        currentNavigationProbe = document.createElement("span");
        currentNavigationProbe.style.display = "none";
        var parent = document.querySelector("div.contents") || document.querySelector("div.wrapper") || document.body;
        parent.appendChild(currentNavigationProbe);
    }
    return probe;
}
;
function clearVisitWaits() {
    if (currentJoinWaitInterval)
        window.clearInterval(currentJoinWaitInterval);
    if (currentJoinTimeout)
        window.clearTimeout(currentJoinTimeout);
    if (currentRaidJoinWait)
        currentRaidJoinWait();
    if (currentCoOpJoinWait)
        currentCoOpJoinWait();
}
;
function visitCoOpJoinPage(callback) {
    var sendCallback = function (result, error) {
        clearVisitWaits();
        if (callback) {
    ...
    doNavigate("http://game.granbluefantasy.jp/#coopraid/offer");
    var registerWait = function () {
        clearVisitWaits();
        currentCoOpJoinWait = waitForElementToExist(document, "div.cnt-coopraid div.prt-input-key input.frm-room-key", sendCallback);
    };
    registerWait();
    var probe = createNavigationProbe();
    currentJoinWaitInterval = window.setInterval(function () {
        if (getNavigationProbe() != probe)
            registerWait();
    }, 33);
    currentJoinTimeout = window.setTimeout(function () {
        log("Join navigation timed out");
        sendCallback(null, "timed out");
        clearVisitWaits();
    }, 5000);
}
;
function visitRaidJoinPage(callback) {
    var sendCallback = function (result, error) {
        clearVisitWaits();
        if (callback) {
            callback(result, error);
    ...
    doNavigate("http://game.granbluefantasy.jp/#quest/assist");
    var registerWait = function () {
        clearVisitWaits();
        currentRaidJoinWait = waitForElementToExist(document, "div.prt-assist div.prt-input-key input.frm-battle-key", sendCallback);
    };
    registerWait();
    var probe = createNavigationProbe();
    currentJoinWaitInterval = window.setInterval(function () {
