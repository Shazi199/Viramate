...
var defaultSettings = {
    ...
    "globalDisable": false,
    "showHardRaids": true,
    "showMagna2Raids": true,
    "showSummonRaids": true,
    "disableButtonAnimations": true,
    "highlightSummons": true,
    "tranquilizeFinger": true,
    "allowGameAjax": true,
    "showQuestInfo": true,
    "hyperlinkUserNames": true,
    "autofocusDropdowns": true,
    "storyTextScale": 1.0,
    "fastCombatAnimations": true,
    "quickJoinFeature": true
};
...
var lastVersion = settings.get("lastVersion") || null;
...
var remoteMessage = JSON.parse(settings.get("remoteMessage") || "null");
var hashesFromVersionJson = JSON.parse(settings.get("hashesFromVersionJson") || "{}");
...
log("Started");
try {
    var patterns = settings.get("remoteShutdownPatterns");
    if (patterns)
        patterns = JSON.parse(patterns);
    if (patterns && patterns.length)
        evaluateRemoteShutdownPatterns(patterns);
    if (isRemoteShutdown) {
        if (!patterns) {
            log("Remote shutdown currently enabled by old version. Starting download check.");
            downloadLatestVersionJson(null, null, function (uid, ok, data) {
                // HACK: Well, let's at least make sure we don't slam the server. Auto-check
                //  will happen later.
                if (!ok)
                    settings.set("remoteShutdownPatterns", "[]");
            });
        }
        else {
            log("Remote shutdown currently enabled for list", patterns);
        }
    }
...
function getAdjustedSettings() {
    ...
    result.allowGameAjax = !result.disableGameAjax;
    result.remoteDisabledFeatures = [];
    result.remoteMessage = remoteMessage;
    for (var i = 0; i < remoteDisabledFeatures.length; i++) {
        var rdf = remoteDisabledFeatures[i];
        if (result[rdf]) {
            result.remoteDisabledFeatures.push(rdf);
            result[rdf] = false;
        }
    }
    ...
    if (result.frameskipRatio > 1)
        result.showPerformanceHud = true;
    result.validHashes = hashesFromVersionJson;
    result.ocache = ocache;
    return result;
}
;
// @ts-ignore
...
function getUserDict(uid) {
    ...
    if (!dict) {
        ...
        try {
            var json = settings.get("lastItems[" + uid + "]");
            if (json)
                dict.lastItems = JSON.parse(json);
        }
        catch (exc) {
            logError(exc);
        }
...
function onRuntimeMessage(msg, sender, sendResponse) {
    ...
    switch (key) {
        ...
        case "getUserIds":
            ...
            break;
        case "getValidHashes":
            sendResponse(hashesFromVersionJson);
            break;
        case "setPassword":
            settings.set("password", msg.password);
            break;
        case "pleaseInjectOhash":
            injectOhashIntoTab(sender, function () {
                sendResponse(tabId);
            });
            return true;
        case "pleaseInjectStylesheets":
            ...
            break;
        case "updateOcache":
            ocache[msg.hash] = msg.hash2;
            break;
        case "getCachedQuestInfo":
            sendResponse(questInfoCache[msg.key] || null);
            break;
        case "setCachedQuestInfo":
            questInfoCache[msg.key] = msg.value;
            break;
        case "heartbeat":
            isDead = false;
            break;
        case "cancelSuspend":
            if (userDict && userDict.isSuspended)
        ...
        case "setGlobalDisable":
            ...
            if (newState !== !!(settings.get("globalDisable") || false)) {
                ...
                if (newState === true)
                    broadcastMessageToAllTabs({ type: "performShutdown" });
            }
            break;
        case "openNewTab":
            chrome.tabs.create({
        ...
        case "getSettings":
            ...
            var forcedVersionUpdate = (lastVersion !== currentVersion);
            if (forcedVersionUpdate) {
                log("Forcing version.json update because manifest version has changed.");
                lastVersion = currentVersion;
                settings.set("lastVersion", currentVersion);
            }
            if (msg.force) {
                log("Forcing version.json update upon request");
                forcedVersionUpdate = true;
            }
            if (!forcedVersionUpdate &&
            ...
            isFreshStart = false;
            return maybeDoUpdate(nextUpdate, updateCheckDelay, getAdjustedSettings, downloadLatestVersionJson, sendResponse, msg.force, tabId, msg.uid);
        case "getRaidCode":
        ...
        case "invalidateRaids":
            ...
            break;
        case "getItemsNoUpdate":
            sendResponse(getUserDict(msg.uid).lastItems);
            break;
        case "getItems":
        ...
        case "invalidateItems":
            if (msg.items) {
                ...
                setLastItemsForUser(msg.uid, null);
            }
            break;
        ...
        case "getUserId":
            ...
            return true;
        case "getIsSuspended":
            sendResponse(userDict && !!userDict.isSuspended);
            return true;
        case "recordRewards":
...
;
function setLastItemsForUser(uid, lastItems) {
    var userDict = getUserDict(uid);
    userDict.lastItems = lastItems;
    settings.set("lastItems[" + uid + "]", JSON.stringify(lastItems));
}
;
...
function handleQuestRewards(msg) {
    ...
    chrome.runtime.sendMessage({
        type: "apiEvent:questResult",
        uid: msg.uid, url: msg.url,
        questId: msg.questId, raidId: raidId,
        data: msg.data
    });
}
;
...
;
// FIXME: Rename this to avoid name collision with injected code?
// @ts-ignore
...
function updateStatus(tabId, uid, callback) {
    ...
    doGameAjax(msg, tabId, uid, function (result, error) {
        getUserDict(uid).nextStatusUpdate = Date.now() + minimumUpdateDelay;
        if (error && (error.indexOf("-- abort") >= 0)) {
            getUserDict(uid).nextStatusUpdate = Date.now() + 2000;
            log("Status update aborted");
        }
        else if (!result) {
...
function downloadLatestVersionJson(tabId, uid, callback) {
            ...
            sendInstallerCommand("installUpdate", function (result, error) {
                ...
                if (result && result.result === "Updated") {
                    if (result.installedVersion.trim().toLowerCase() !==
                        ...
                        log("Automatic update installed v" + result.installedVersion);
                    }
                    else {
    ...
    try {
        ...
        var versionPrefix = "https://viramate.luminance.org";
        if (false)
            versionPrefix = "http://luminance.org/vm";
        xhr.open("GET", versionPrefix + "/version.json?_=" + Date.now(), true);
        xhr.timeout = 15 * 1000;
        xhr.onload = function () {
            if (xhr.status === 200)
...
;
function evaluateRemoteShutdownPatterns(patterns) {
    log("Checking remote shutdown status against saved list");
    settings.set("remoteShutdownPatterns", JSON.stringify(patterns));
    var currentVersion = chrome.app.getDetails().version;
...
function parseVersionJson(json) {
    try {
        var didJsonChange = json !== settings.get("lastParsedVersionJson");
        var dict = JSON.parse(json);
        var currentVersion = chrome.app.getDetails().version;
        log("Processing latest version.json. It appears " + (didJsonChange ? "to have changed." : "unchanged."));
        if (dict.shutdown_versions && dict.shutdown_versions.length)
            evaluateRemoteShutdownPatterns(dict.shutdown_versions);
        ...
        if (dict.message)
            log(dict.message);
        settings.set("remoteMessage", JSON.stringify(dict.message || null));
        ...
        if (JSON.stringify(hashesFromVersionJson || {}) !== JSON.stringify(dict.hashes)) {
            log("Hash list from version json changed, disabling local compatibility shutdown");
            isShutdown = false;
        }
        settings.set("hashesFromVersionJson", JSON.stringify(dict.hashes || {}));
        hashesFromVersionJson = dict.hashes;
        if (didJsonChange)
            settings.set("lastVersionJsonChange", Date.now());
        settings.set("lastParsedVersionJson", json);
        return true;
    }
    catch (exc) {
...
;
function injectOhashIntoTab(sender, callback) {
    var injectScript = function (uri, cb) {
        try {
            chrome.tabs.executeScript(sender.tab.id, { file: uri, runAt: "document_start" }, cb);
    ...
    if (isRemoteShutdown || currentSettings.isShutdown)
        return;
    injectScript("/content/sha_dev.js", function () {
        injectScript("/content/esprima.js", function () {
            injectScript("/content/escodegen.browser.min.js", function () {
                injectScript("/build/oppenheimer.js", callback);
            });
        });
    });
}
...
function injectStylesheetsIntoTab(sender) {
    var versionRegex = /Chrome\/([0-9]+)\.([0-9]+)/g;
    var m = versionRegex.exec(navigator.userAgent);
    if (m && m[1]) {
        var majorVersion = parseFloat(m[1]);
        if (majorVersion >= 73) {
            chrome.pageAction.setPopup({
                tabId: sender.tab.id,
                popup: chrome.extension.getURL("src/popup/popup.html")
            });
            if (!loggedPopupWarning) {
                loggedPopupWarning = true;
                log("Enabling popup workaround for bug in Chrome 73");
            }
        }
    }
    ...
    if (currentSettings.keyboardShortcuts3 || currentSettings.keyboardShortcutsCombat)
    ...
    if (currentSettings.fastCombatAnimations)
        injectStylesheet("/css/fast-combat-animations.css");
