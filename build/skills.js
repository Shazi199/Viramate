...
function onQuickSkillClick(evt) {
    ...
    var characterNumber = pe.getAttribute("ability-character-num");
    if (combatState.now_pos != characterNumber) {
        sendExternalMessage({
            type: "changeCommandSlide",
            nowPos: characterNumber
        });
...
function onRailAbilityClick(evt) {
    ...
    for (var i = 0; i < buttons.length; i++) {
        ...
        buttons[i].classList.remove("tmp-mask");
...
function tryPerformOneClickSummon(expectedSkillName) {
    // HACK
    logError("One-click summons are disabled");
    return false;
    if (!expectedSkillName)
        return false;
    ...
    if (actualSkillName !== expectedSkillName) {
        logError("Expected summon skill '" + expectedSkillName +
...
function processSkillHotkey(evt) {
    ...
    if (!Number.isNaN(characterNumber) && (characterNumber >= 1) && (characterNumber <= 5)) {
        ...
        return trySelectCharacterByNumber(characterNumber, true);
    }
...
function getSelectedCharacterNumber() {
    var characterPanels = queryEncryptedAll(document, "div.prt-command-chara:not(.quick-panels)");
...
;
function trySelectCharacterByNumber(number, selectForQuickPanels, resultId) {
    // FIXME: If this is called too quickly after a previous call, the panel just gets closed
    ...
    if (number === 5) {
        ...
        if (button) {
            ...
            pulseElement(button);
            generateClick(button, false, resultId);
            return null;
        }
    }
    var isAnyCommandPanelOpen = Array.from(queryEncryptedAll(document, "div.prt-command-chara:not(.quick-panels)"))
        .some(function (e) { return e.style.display === "block"; }) ||
        (queryEncrypted(document, "div.prt-command-summon").className.indexOf("summon-show") >= 0);
    if (isAnyCommandPanelOpen) {
        var selector = 'div.prt-command-chara[pos="' + number + '"]';
...
function setFocusedCharacterIndex(index) {
    ...
    var quickPanels = queryEncryptedAll(document, 'div.quick-panel');
    for (var i = 0; i < quickPanels.length; i++)
        removeEncryptedClass(quickPanels[i], "focused");
    if (index !== null) {
        var quickPanel = queryEncrypted(document, 'div.quick-panel[index="' + index + '"]');
        // ????
        if (!quickPanel)
            throw new Error("Couldn't find quick panel");
        addEncryptedClass(quickPanel, "focused");
    }
...
function tryActivateAbilityByIndex(index) {
    ...
    var jita = (selectedCharacterNumber <= 1) ||
        (parsed && (parsed.id[0] == '9'));
    if ((index <= 0) && jita) {
        log("Hotkey disabled");
        return false;
    }
    if (parsed) {
        var selector = "div.quick-panel div." + parsed.icon.className.replace(/ /g, ".");
        var quickSkillElement = queryEncrypted(document, selector);
        if (!quickSkillElement && currentSettings.showQuickPanels)
            logError("Couldn't find visible skill button with selector", selector);
        var btn = quickSkillElement || parsed.icon;
        pulseElement(btn);
        if (!normalAttacking) {
...
;
function ensureQuickSummonPanelExists() {
    if (!currentSettings.showQuickPanels)
        return false;
    var cls = encryptClassName("quick-summon-panel");
    if (quickSummonPanel && !document.contains(quickSummonPanel))
        quickSummonPanel = null;
    if (!quickSummonPanel)
        quickSummonPanel = document.querySelector("div." + cls);
    if (!quickSummonPanel) {
        quickSummonPanel = document.createElement("div");
        quickSummonPanel.className = cls;
    }
    ...
    var hasRightParent = quickSummonPanel.parentNode &&
        (quickSummonPanel.parentNode.className === "prt-command");
    if (!hasRightParent) {
        var commandPanel = document.querySelector("div.prt-command");
        if (commandPanel) {
            injectElement(commandPanel, quickSummonPanel, false);
        }
        else {
            var tempContainer = document.querySelector("div.contents");
            injectElement(tempContainer, quickSummonPanel, false);
        }
    }
    return !!quickSummonPanel;
}
function quickSummonMouseDownHandler(evt) {
    if (evt && (evt.target.className.indexOf(encryptClassName("unavailable")) >= 0))
        suppressEvent(evt);
}
;
function updateQuickSummonPanel() {
    ...
    for (var i = 0; i < summons.length; i++) {
        ...
        quickSummon.className = cls + " " + encryptClassName(isAvailable ? "available" : "unavailable");
        ...
        quickSummon.addEventListener("mousedown", quickSummonMouseDownHandler, true);
...
function ensureQuickPanelsContainerExists() {
    if (!currentSettings.showQuickPanels)
        return false;
    var cls = encryptClassName("quick-panels");
    injectStylesheet("quick-panels.css", document.head);
    if (quickPanels && !document.body.contains(quickPanels))
        quickPanels = null;
    if (!quickPanels)
        quickPanels = document.querySelector("div." + cls);
    if (!quickPanels) {
        quickPanels = document.createElement("div");
        var className = "prt-command-chara " + cls;
        quickPanels.className = className;
    }
    // HACK: If we put this in the party panel, it auto-hides, but then it comes before
    //  the actual character panels in the query order and breaks things
    var hasRightParent = quickPanels.parentNode &&
        (quickPanels.parentNode.className === "prt-command");
    if (!hasRightParent) {
        var commandPanel = document.querySelector("div.prt-command");
        if (commandPanel) {
            injectElement(commandPanel, quickPanels, false);
        }
        else {
            var tempContainer = document.querySelector("div.contents");
            injectElement(tempContainer, quickPanels, false);
        }
    }
    return !!quickPanels;
}
function canInPlaceUpdateTree(target, source) {
    if (!source || !target)
        return false;
    ...
    for (var i = 0; i < target.childNodes.length; i++) {
        if (!canInPlaceUpdateTree(target.childNodes[i], source.childNodes[i]))
            return false;
    }
    return true;
}
;
function inPlaceUpdateElement(target, source) {
    var wasOn = target.classList.contains("on");
    var sourceAttributeNames = new Set();
    for (var i = 0; i < source.attributes.length; i++) {
        var attr = source.attributes[i];
        if (attr.name === "evt-registered")
            continue;
        sourceAttributeNames.add(attr.name);
...
;
function inPlaceUpdateTree(target, source) {
    if (target.attributes)
        inPlaceUpdateElement(target, source);
    for (var i = 0; i < target.childNodes.length; i++) {
        var tn = target.childNodes[i];
        var sn = source.childNodes[i];
        inPlaceUpdateTree(tn, sn);
    }
}
;
var isEventListenerRegisteredMap = new WeakMap();
var heldButtonState = new WeakMap();
function processPartyMember(index, partyIcon, skillContainer, updateQuickPanels, pid) {
    ...
    if (updateQuickPanels) {
        if (ensureQuickPanelsContainerExists()) {
            var quickPanel = queryEncrypted(document, 'div.quick-panel[index="' + (index+1) + '"]');
            if (!quickPanel) {
                quickPanel = document.createElement("div");
                className = encryptClassName("quick-panel") + " " +
                    // HACK: So some of the built-in styles apply
                    "prt-ability-list";
                quickPanel.className = className;
                quickPanel.setAttribute("index", (index+1));
                var x = (index * 54);
    ...
    if (quickPanel) {
        if ((updateQuickPanels === "reset") || isDead)
            ...
            existingButtons = quickPanel.querySelectorAll("div.lis-ability");
    }
    ...
    for (var i = 0, l = 4; i < l; i++) {
        ...
        if (parsed) {
            ...
            if (currentSettings.keyboardShortcutsCombat) {
                if ((index > 0) || (i > 0))
        ...
        if (updateQuickPanels) {
            ...
            if (existingButtons)
                existingButton = existingButtons[i];
            var isHeld = existingButton && (heldButtonState.get(existingButton) === true);
            // FIXME: This confuses the hell out of cygames' event handlers
            var canInPlaceUpdate = canInPlaceUpdateTree(existingButton, btn) &&
                isHeld;
            var quickButton;
            if (canInPlaceUpdate) {
                log("In-place updating held button");
                inPlaceUpdateTree(existingButton, btn);
                quickButton = existingButton;
            }
            else {
                quickButton = btn.cloneNode(true);
                quickButton.removeAttribute("evt-registered");
                quickButton.addEventListener("mousedown", function () { heldButtonState.set(quickButton, true); }, true);
                quickButton.addEventListener("mouseup", function () { heldButtonState.set(quickButton, false); }, true);
                quickButton.addEventListener("mouseleave", function () { heldButtonState.set(quickButton, false); }, true);
            }
            if (isAbilityLocked)
            ...
            if (existingButton) {
                if (!canInPlaceUpdate)
                    quickPanel.replaceChild(quickButton, existingButton);
            }
            else {
            ...
            if (parsed) {
                ...
                if (isEventListenerRegisteredMap.get(quickButton) !== true) {
                    isEventListenerRegisteredMap.set(quickButton, true);
...
function trySelectNinjaMark(keyText) {
    var index = null;
    var isNinja = document.querySelector("div.lis-ninja-mark");
    var lookup;
    var remap = {
        "ARROWUP": "W",
        "ARROWRIGHT": "D",
    ...
    var keyName = (remap[keyText] || keyText).replace("KEY", "");
    var indexFromKeyName = lookup[keyName];
    if ((indexFromKeyName >= 1) && (indexFromKeyName <= 4)) {
        var selector;
        if (isNinja) {
            selector = "div.lis-ninja-mark.mark" + indexFromKeyName;
        }
        else {
            selector = "div.lis-ability-mark.mark" + indexFromKeyName + " .lis-ability-frame";
        }
...
function tickPartyUI(partyIcons, skillContainers) {
    if (!currentSettings.showQuickPanels)
        return;
    if (!quickPanels)
        quickPanels = queryEncrypted(document, "div.quick-panels");
    if (!quickPanels)
        return;
    if (!commandTop)
    ...
    if (!commandTop)
        return;
    var className = "prt-command-chara " + encryptClassName("quick-panels") +
        " " + encryptClassName(commandTop.style.display !== "none"
    ...
    if (pendingOneClickSummon && false)
